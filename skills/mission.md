---
description: Run a governed multi-agent mission from a grilled goal to a verified, recoverable deliverable.
...
6. **EXECUTE.** Read the frozen plan as a parsed object and run `~/.claude/workflows/mission-executor.workflow.js` through the Workflow tool with `args: { plan: <parsed plan>, completed: {} }`. Do not substitute an improvised executor.

7. **AUDIT and DELIVER.** Treat the executor's fresh integrated audit as canonical. A failed audit blocks delivery; `human_required` must name every deferred criterion. Inspect the complete diff, remove temporary scaffolding, compare failures with baseline, and rerun a witness only when the audit identifies an unobserved claim. Write `.mission/<run-id>/REPORT.md`; it is the completion marker and heartbeat disarm signal. Deliver only within the constitution's human-authority boundary.

For `--resume <run-id>`, load the frozen plan, journal, branch commits, and witness evidence. Confirm the process identity in `mission.lock` is no longer live, then claim the run by recording the current `CLAUDE_PID`, process start time, and `CLAUDE_CODE_SESSION_ID`. Reconstruct completed nodes only from committed closure evidence and pass `args: { plan, completed }` to the canonical executor. Do not repeat GRILL or rewrite the brief. If the frozen plan is invalid, stop and name the premise requiring human reopening.
